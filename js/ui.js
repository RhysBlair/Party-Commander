...
// 골드 단위 축약: 숫자만 반환 (1000→1K, 1000000→1M, 1000000000→1B)
function fmtGNum(n) {
  n = Math.floor(n);
...
// G를 노란색으로 붙인 형태 (HTML)
function fmtG(n) {
  return fmtGNum(n) + '<span style="color:#e2b96f">G</span>';
}

// 버프 원인 태그 — 해당 stat에 영향 주는 활성 버프를 뱃지로 반환
function statBuffTag(char, statKey) {
  const b = char.activeBuffs;
  const tags = [];
  const hasPB = b && (b.statMult?.timer || 0) > 0;
  const pbTxt = hasPB ? `파워버스트 ×${(b.statMult.mult||1).toFixed(1)}` : '';

  switch (statKey) {
    case 'atk':
      if (hasPB)                              tags.push([pbTxt,                                           '#ff7043']);
      if (b && (b.atk?.timer||0) > 0)        tags.push([`${b.atk.label||'공격력'} ×${(b.atk.mult||1).toFixed(1)}`, '#ff9800']);
      break;
    case 'physDef':
    case 'magicDef':
    case 'evade':
    ...
    case 'maxHp':
      if (hasPB)                              tags.push([pbTxt,                                           '#ff7043']);
      if (b && (b.hp?.timer||0) > 0)         tags.push([`하이퍼바디 ×${(b.hp.mult||1).toFixed(1)}`,      '#e74c3c']);
      break;
    case 'critRate':
      if (hasPB) tags.push([pbTxt, '#ff7043']);
      break;
    case 'critDmg':
      if (b && (b.critDmg?.timer||0) > 0)   tags.push([`샤프아이즈 +${(b.critDmg.bonus||0).toFixed(1)}`, '#f1c40f']);
      break;
  }
  if (!tags.length) return '';
  return ' ' + tags.map(([txt, col]) =>
    `<span style="color:${col};font-size:8px;background:#0d0d0d;border:1px solid ${col}55;border-radius:3px;padding:0 3px;white-space:nowrap">${txt}</span>`
  ).join(' ');
}

// 직업별 공격 라벨
function atkLabel(char) {
...
function buffIconsHtml(char) {
  ...
  if ((upg.atk_boost || 0) > 0)
    list.push(['🗡️', '공격강화(축복)', `공격력 +${upg.atk_boost * 5}%`]);
...
function updateHUD() {
  const s = gameState;
  document.getElementById('hud-gold').innerHTML = `골드: ${fmtG(s.gold)}`;
          ...
          <div class="char-col-left">
            ...
            <div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;margin:3px 0">
              ...
              <button id="freset-${char.id}" class="reset-stat-btn${canReset ? '' : ' disabled'}" style="font-size:10px"
                      ...
                      title="골드 ${fmtGNum(resetCost)}G 소모">스텟초기화</button>
            </div>
        ...
        const enhBtn  = _iMax === 0 ? ''
          : item.enhance < _iMax
            ? `<button class="small-btn enhance-btn ${canAff ? '' : 'disabled'}"
                       onclick="this.disabled=true;showEnhanceFloat(this,tryEnhanceEquipment(${item.uid}));markTabDirty();"
                       title="성공률 ${succPct}%">
                 강화 ${fmtG(enhanceCost(item))} (${succPct}%)
               </button>`
...
function renderShopTab() {
  ...
  function potionMini(potionId) {
    ...
    const buyBtns = [100, 500].map(qty => {
      ...
      return `<button class="potion-buy-btn ${canAff ? '' : 'disabled'}"
                ...
                +${qty}<br><span style="font-size:9px;color:#aaa">${fmtG(cost)}</span>
              </button>`;
    }).join('');
    ...
    return `
      <div class="potion-mini-card">
        ...
        <div style="font-size:10px;color:${col};margin:2px 0">${label} · ${fmtG(p.cost)}/개</div>
  ...
  el.innerHTML = `
    <div class="eq-section-title">
      물약 상점 <span style="color:#e2b96f;font-size:11px"> 골드: ${fmtG(gameState.gold)}</span>
    </div>
...
function showOfflineModal(result) {
  ...
  document.getElementById('offline-gold').innerHTML = fmtG(result.totalGold);
...
function buildModalEquipment(char) {
  ...
  const slots = slotList.map(slot => {
    ...
    const enhBtn = item && _mMax > 0 && item.enhance < _mMax ? (() => {
      ...
      return `<button class="small-btn enhance-btn ${canAff ? '' : 'disabled'}"
                      ...
                      title="성공률 ${succPct}%">강화 ${fmtG(cost)} (${succPct}%)</button>`;
  ...
  const invHtml = equipableInv.length === 0
    ...
    : equipableInv.map(item => {
        ...
        const sellPrice = e.cost ? Math.floor(e.cost * 0.6) : 0;
        const sellBtn = sellPrice > 0
          ? `<button class="equip-remove sell-btn" onclick="trySellItem(${item.uid});renderCharModalBody();">판매 ${fmtG(sellPrice)}</button>`
          : '';
        ...
        const enhBtn  = _invMax === 0 ? ''
          : item.enhance < _invMax
            ? `<button class="small-btn enhance-btn ${canAff ? '' : 'disabled'}"
                       onclick="this.disabled=true;showEnhanceFloat(this,tryEnhanceEquipment(${item.uid}));markCharModalDirty();">강화 ${fmtG(enhanceCost(item))}</button>`
            : `<span style="color:#e2b96f;font-size:10px">MAX</span>`;
        return `
          <div class="inv-item">
